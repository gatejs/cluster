...
Server.prototype._bindEventsSocket = function (sock) {
	...
	// save the original fast-tcp message manager
	socket._orgMessage = socket._onMessage.bind(socket);

	socket.on('close', function () {
		if(socket._type == Serializer.MT_DOWNLINK) {
			// event downlink
			_this._superEmit('downlink/del', socket);
	...
	function authentification() {
		// generate a seed
		socket._seed = jen.password(32,32);

		// compute hmac in advance
		const hmac = crypto.createHmac('sha256', _this.psk);
		hmac.update(socket._seed);
		socket._hmac = hmac.digest('hex')

		// replace the message manager
		socket._onMessage = ((msg) => {
			if(msg.mt != Serializer.MT_AUTH_HMAC) {
				socket._onError(ut.error("Sending command before MT_AUTH_HMAC got "+msg.mt))
				return(socket.destroy());
			}
			if(msg.data !== socket._hmac) {
				socket._onError(ut.error("Wrong HMAC authentification"))
				return(socket.destroy());
			}
		...
		socket._send('', socket._seed, Serializer.MT_AUTH_SEED);
	}

	// once authed run addressing step
	function addressing() {
		// replace again the message manager
		socket._onMessage = ((msg) => {

			// it's a downlink
			if(msg.mt == Serializer.MT_DOWNLINK) {
				socket._type = Serializer.MT_DOWNLINK;
				_this.sockets[socket.id] = socket;

				// Sends the id to socket client
				socket._send('', socket.id, Serializer.MT_REGISTER);

				// client typed, restore MT routing
				socket._onMessage = socket._orgMessage;

				/**
				* A new Downlink was connected.
				*
				* @event Server#downlink/add
				*/
				_this._superEmit('downlink/add', socket);
			}
			// it's an uplink
			else if(msg.mt == Serializer.MT_UPLINK) {
				if(!_this._router) {
					socket._onError(ut.error("Receive new uplink client but no router defined: new Router()"))
					return(socket.destroy());
				}
				socket._type = Serializer.MT_UPLINK;
				socket.hostname = msg.data;

				// Sends the id to socket client
				socket._send('', _this._router.hostname, Serializer.MT_UPLINK);
				socket._send('', socket.id, Serializer.MT_REGISTER);

				// add sock to reflector
				_this._router.reflector(socket)
			}
			else {
				socket._onError(ut.error("Waiting for MT DOWN or UP Link and receive "+msg.mt))
				return(socket.destroy());
			}
		});
	}

	// when socket connects send required authentification
	if(this.psk) authentification();
	else addressing();
