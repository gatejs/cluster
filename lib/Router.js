...
Router.prototype.reflector = function (sock) {
	...
	sock._onMessage = (msg) => {
		...
		if(msg.mt == Serializer.MT_UL_NEW_LINK) {
			...
			//self._sendUplinksBroadcast(msg.cells, '', msg.data, Serializer.MT_UL_NEW_LINK);
			...
			self._sendUplinksBroadcast(msg.cells, '', msg.data, Serializer.MT_UL_JOIN_ROOM);
		}
			...
			self._sendUplinksBroadcast(msg.cells, '', msg.data, Serializer.MT_UL_LEAVE_ROOM);
		}
		else {
...
Router.prototype._sendUplinksBroadcast = function (cells, event, data, mt, opts) {
	const selected = []
	const sent = {};
