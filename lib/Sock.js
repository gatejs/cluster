...
function Sock(serializer, opts) {
  ...
  this._cost = ut.isNumber(opts.cost) ? opts.cost : 1;
...
Sock.prototype._send = function (event, data, mt, opts) {
	...
	if(!cluster) {
		if(this._router && this._router.hostname) cluster = this._router.hostname
	}

	const buff = this._serializer.serialize(event, data, mt, messageId, cluster);
