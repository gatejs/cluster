...
Serializer.CELL_ADD = 1;
Serializer.CELL_SUB = 2;
...
Serializer.MT_AUTH_SEED = 1;
Serializer.MT_AUTH_HMAC = 2;
Serializer.MT_UPLINK = 3;
Serializer.MT_DOWNLINK = 4;
Serializer.MT_REGISTER = 5;

// nodes commands
Serializer.MT_DATA = 100;
Serializer.MT_DATA_TO_SOCKET = 101;
Serializer.MT_DATA_TO_ROOM = 102;
Serializer.MT_DATA_BROADCAST = 103;
Serializer.MT_DATA_WITH_ACK = 104;
Serializer.MT_ACK = 105;
Serializer.MT_JOIN_ROOM = 106;
Serializer.MT_LEAVE_ROOM = 107;
Serializer.MT_LEAVE_ALL_ROOMS = 108;
Serializer.MT_DATA_STREAM_OPEN = 109;
Serializer.MT_DATA_STREAM = 110;
Serializer.MT_DATA_STREAM_CLOSE = 111;
Serializer.MT_DATA_STREAM_OPEN_WITH_ACK = 112;
Serializer.MT_DATA_STREAM_OPEN_TO_SOCKET = 113;
Serializer.MT_DATA_STREAM_OPEN_TO_ROOM = 114;
Serializer.MT_DATA_STREAM_OPEN_BROADCAST = 115;

// Uplink commands
Serializer.MT_UL_NEW_LINK = 200;
Serializer.MT_UL_DEL_LINK = 201;
Serializer.MT_UL_JOIN_ROOM = 202;
Serializer.MT_UL_LEAVE_ROOM = 203;


Serializer.prototype.serialize = function (event, data, mt, messageId, cells, policy) {
	var dt;
	...
	return this._serialize(event, data, mt, messageId, dt, cells, policy);
};
...
Serializer.prototype.deserialize = function (buff) {
	...
	// Cells forwarding policy
	var policy = buff[offset++];

	// Cluster source message
	var cellsLength = buff.readUInt8(offset);
	offset += 1;

	var cells = buff.toString(undefined, offset, offset + cellsLength);
	offset += cellsLength;
	...
	return { event: event, data: data, messageId: messageId, mt: mt, cells: cells, policy: policy };
};

Serializer.prototype._serialize = function (event, data, mt, messageId, dt, cells, policy) {
	if(!cells) cells = '';

	var cellsLength = Buffer.byteLength(cells);
	var eventLength = Buffer.byteLength(event);
	var dataLength = this._getDataLength(data, dt);

	// version(1), flags(1), dt(1), mt(1), policy(1), cellsLength(1), cells(), messageId(4),
	// eventLength(2), event(), dataLength(4), data()
	var messageLength =  6 + cellsLength + 6 + eventLength + 4 + dataLength;
	...
	// Cells forwarding policy
	buff[offset] = policy === Serializer.CELL_SUB ? policy : Serializer.CELL_ADD;
	offset++;

	// Cells message
	buff.writeUInt8(cellsLength, offset);
	offset += 1;

	buff.write(cells, offset, cellsLength);
	offset += cellsLength;
