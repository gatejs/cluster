...
Serializer.prototype.serialize = function (event, data, mt, messageId, cluster) {
	var dt;
	...
	return this._serialize(event, data, mt, messageId, dt, cluster);
};
...
Serializer.prototype.deserialize = function (buff) {
	...
	var clusterLength = buff.readUInt8(offset);
	offset += 1;

	var cluster = buff.toString(undefined, offset, offset + clusterLength);
	...
	return { event: event, data: data, messageId: messageId, mt: mt, cluster: cluster };
};

Serializer.prototype._serialize = function (event, data, mt, messageId, dt, cluster) {
	if(!cluster) cluster = '';

	var clusterLength = Buffer.byteLength(cluster);
	var eventLength = Buffer.byteLength(event);
	var dataLength = this._getDataLength(data, dt);

	// version(1), flags(1), dt(1), mt(1), clusterLength(1), cluster(), messageId(4),
	// eventLength(2), event(), dataLength(4), data()
	var messageLength =  5 + clusterLength + 6 + eventLength + 4 + dataLength;
	...
	// Cluster source message
	buff.writeUInt8(clusterLength, offset);
	offset += 1;

	buff.write(cluster, offset, clusterLength);
	offset += clusterLength;
